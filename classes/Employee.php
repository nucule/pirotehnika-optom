...
class EmployeeCore extends ObjectModel
{
    ...
    public static function getEmployeesByShop($active_only = true, $id_shop = 0)
    {
        return Db::getInstance()->executeS('
			SELECT e.`id_employee`, CONCAT(e.`lastname`, \' \', e.`firstname`) as name
			FROM `'._DB_PREFIX_.'employee` e
			RIGHT JOIN '._DB_PREFIX_.'employee_shop es ON (es.id_employee = e.id_employee AND es.id_shop = '.$id_shop.')
			'.($active_only ? ' WHERE `active` = 1' : '').'
			ORDER BY `lastname` ASC
		');
    }

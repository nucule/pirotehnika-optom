...
//  public $price_discount = 0.000000;
    /** @var float Ecotax */
    public $ecotax = 0;
...
// Иванов   
    /** @var int(1) Единица измерения для продажи */
    public $sale_unity;

    /** @var int(1) Единица измерения поставщика */
    public $parent_sale_unity;

    /** @var decimal 20,6 Коэффициент цена и количества */
    public $sale_unit_price_ratio;

    /** @var varchar 20 Описание фасовки */
    public $sale_unity_pack;

    /** @var int(4) Штук в упаковке */
    public $r1;

    /** @var int(4) Упаковке в блоке*/
    public $r2;

    /** @var int(4) Единиц измерения в коробке */
    public $r3;

    /** @var int Страна происхождения */
    public $country_prois;
...
// Иванов ^
    ...
    /*** Флаг, указывающий на то, что данные в карточке товара проверены */
    public $is_checked;
    ...
    public static $definition = array(
        ...
        'fields' => array(
            ...
            'objem' =>                    array('type' => self::TYPE_FLOAT, 'validate' => 'isUnsignedFloat'),
            'quantity_discount' =>            array('type' => self::TYPE_BOOL, 'validate' => 'isBool'),
            'ean13' =>                        array('type' => self::TYPE_STRING, 'validate' => 'isEan13', 'size' => 13),
            'ean13_2' =>                        array('type' => self::TYPE_STRING, 'validate' => 'isEan13', 'size' => 13),
            'ean13_3' =>                        array('type' => self::TYPE_STRING, 'validate' => 'isEan13', 'size' => 13),
            'r1' =>                             array('type' => self::TYPE_INT, 'validate' => 'isUnsignedId'),
            'r2' =>                             array('type' => self::TYPE_INT, 'validate' => 'isUnsignedId'),
            'r3' =>                             array('type' => self::TYPE_INT, 'validate' => 'isUnsignedId'),
            'sg' =>                             array('type' => self::TYPE_INT, 'validate' => 'isUnsignedId'),
            'sgt' =>                array('type' => self::TYPE_STRING, 'validate' => 'isString'),
            'country_prois' =>                array('type' => self::TYPE_INT, 'validate' => 'isUnsignedId'),
            'tip' =>                array('type' => self::TYPE_INT, 'validate' => 'isUnsignedId'),
            'action' =>            array('type' => self::TYPE_BOOL, 'validate' => 'isBool'),
            'rname' =>                array('type' => self::TYPE_STRING, 'validate' => 'isString'),
            'rtor' =>                array('type' => self::TYPE_INT, 'validate' => 'isUnsignedId'),
            'rprice' =>                        array('type' => self::TYPE_FLOAT, 'validate' => 'isPrice'),
            'r_sale_unity' =>                array('type' => self::TYPE_INT, 'validate' => 'isUnsignedId'),
            'forag' =>            array('type' => self::TYPE_BOOL, 'validate' => 'isBool'),
            'web_url' =>                array('type' => self::TYPE_STRING, 'validate' => 'isString'),
            'sert' =>                array('type' => self::TYPE_STRING, 'validate' => 'isString'),
            'sert_full' =>                array('type' => self::TYPE_STRING, 'validate' => 'isString'),
            'gtd' =>                array('type' => self::TYPE_STRING, 'validate' => 'isString'),
            'max_discount' =>                array('type' => self::TYPE_FLOAT, 'validate' => 'isFloat'),
            ...
            'sale_unity' =>                  array('type' => self::TYPE_INT, 'shop' => true, 'validate' => 'isUnsignedInt'),
            'parent_sale_unity' =>          array('type' => self::TYPE_INT, 'shop' => true, 'validate' => 'isUnsignedInt'),
            'sale_unit_price_ratio' =>            array('type' => self::TYPE_FLOAT, 'shop' => true),
            'sale_unity_pack' =>                  array('type' => self::TYPE_STRING, 'shop' => true, 'validate' => 'isString'),
            ...
            'is_checked' =>        array('type' => self::TYPE_BOOL, 'shop' => true, 'validate' => 'isBool'),
    ...
    public function __construct($id_product = null, $full = false, $id_lang = null, $id_shop = null, Context $context = null)
    {
        ...
        $this->my_is_show=(int)Configuration::get('PS_IS_SHOW_PRODUCTS',null,null,$id_shop);
   }
    ...
    public function getIsChecked()
    {
        $sql = 'SELECT is_checked
                ...
                WHERE product_shop.`active` = '.$this->id;
    ...
    */
    public static function getNewProducts($id_lang, $page_number = 0, $nb_products = 10, $count = false, $order_by = null, $order_way = null, Context $context = null)
    {
        ...
        $isShow = (int)Configuration::get('PS_IS_SHOW_PRODUCTS');
        if ($count) {
            $sql = 'SELECT COUNT(p.`id_product`) AS nb
                ...
                '.($isShow ? '' : ' AND IFNULL(stock.quantity, 0) > 0 ' ).'
        ...
        $sql->join(Shop::addSqlAssociation('stock_available', 'stock'));
        ...
        $sql->where(((int)Configuration::get('PS_IS_SHOW_PRODUCTS') ? '' : 'IFNULL(stock.quantity, 0) > 0' ));
        if ($front) {
...
//        return Product::getProductsProperties($id_lang, $result);
        ...
        if (Configuration::get('PS_IS_SHOW_DISCOUNT',null,null,(int)$context->shop->id) && $context->customer->show_skid) {
            foreach($result222 as &$e){
                ...
                } elseif ($e['country_prois'] == 2) {
                    $e['price_discount'] =  round($e['price'] * (100 - MIN($context->customer->skid_2, $e['max_discount'])) / 100, 2);
                } elseif ($e['country_prois'] == 3) {
                    $e['price_discount'] =  round($e['price'] * (100 - MIN($context->customer->skid_3, $e['max_discount'])) / 100, 2);
                } else {
    ...
    */
    ...
    public static function getIsShowPriceDisc() {
        $context = Context::getContext();
        return Configuration::get('PS_IS_SHOW_DISCOUNT',null,null,(int)$context->shop->id) && $context->customer->show_skid;
    }
    
    public static function getIsShowPriceWoDisc() {
        $context = Context::getContext();
        return $context->customer->show_price_wo_skid;
//      return false;
    }
    ...
    public static function getPriceDisc($id_product = 0) {
        ...
        if (Configuration::get('PS_IS_SHOW_DISCOUNT',null,null,(int)$context->shop->id) && $context->customer->show_skid) {
            if ($product->country_prois == 1) {
                ...
                return  round(Product::getPriceStatic((int)$product->id, true, null, 6, null, false, true, 1) * (100 - MIN($context->customer->skid_2, $product->max_discount)) / 100, 2);
            } elseif ($product->country_prois == 3) {
                return  round(Product::getPriceStatic((int)$product->id, true, null, 6, null, false, true, 1) * (100 - MIN($context->customer->skid_3, $product->max_discount)) / 100, 2);
            } else {
    ...
    public static function getPriceRetail($id_product = 0) {
            $context = Context::getContext();
            $sql = new DbQuery();
        ...
        $res = round(Db::getInstance(_PS_USE_SQL_SLAVE_)->getValue($sql),2);
